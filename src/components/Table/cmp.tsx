...
const Table = ({ columns, data, bordered, ...rest }: TableProps & StyledTableProps) => {
  ...
  const sortedData = React.useMemo(() => (
    ...
    ? data
    : data.sort((a, b) => {
      const column = columns.find(({ label }) => label === sortedColumn.column)
      if(!column)
        return 0
            ...
            columns.map(({ sortable, label }, i) => (
              <th key={i} 
                  ...
                  onClick={() => setSortedColumn({ 
                    column: label, 
                    direction: toggleSort(sortedColumn.direction as SortDirection) 
                  })}>
                { sortable && 
                  <div style={{opacity: isSortedColumn(label) ? 1 : .45, display: "inline-block", paddingRight: "1rem"}}>
                    <Icon name={
                      !isSortedColumn(label) ?
                      "sort"
                      : sortedColumn.direction === "asc" ? "sort-up" : "sort-down"
                    } />
                  </div>
                }
                { label }
